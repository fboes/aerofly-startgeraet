...
Be aware that calling \`set-flightplan-waypoints\` might yet again reposition the aircraft.`,
        inputSchema: {
            ...
            speed_kts: z
                ...
                .nonnegative()
                .optional()
                .describe(`Initial airspeed in knots. If not set: defaults to \`0\` when \`configuration\` is \`OnGround\`, otherwise defaults to the aircraft's current cruise speed.`),
...
- \`ShortFinal\` / \`Final\`: Landing approach configuration (flaps/gear extended).`),
    ...
    server.registerTool("set-mission-briefing", {
...
`),
    ...
    server.registerTool("get-skyvector-url", {
        ...
        return returnMcpToolResult(new SkyVectorUrl(flightService.getAeroflyFlight())

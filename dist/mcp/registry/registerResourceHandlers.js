...
function registerResources(server) {
    server.registerResource("aircraft", RESOURCE_AIRCRAFT, {
    ...
    }, (uri) => returnMcpResourceResult(AeroflyFlightMcpResourceService.getAircraftList(), uri, MIME_TYPE_RESPONSE));
    server.registerResource("aircraft-detail", new ResourceTemplate(`${RESOURCE_AIRCRAFT}/{aeroflyCode}`, {
        list: () => ({
            resources: AeroflyFlightMcpResourceService.getAircraftRessources(),
        }),
    }), {
        ...
        mimeType: MIME_TYPE_RESPONSE,
    }, (uri, { aeroflyCode }) => returnMcpResourceResult(AeroflyFlightMcpResourceService.getAircraft(String(aeroflyCode)), uri, MIME_TYPE_RESPONSE));
    server.registerResource("aircraft-tags", RESOURCE_AIRCRAFT_TAGS, {
        description: `A list of all tags which aircraft in Aerofly FS 4 can be searched by.`,
        mimeType: MIME_TYPE_RESPONSE,
    }, (uri) => returnMcpResourceResult(AeroflyFlightMcpResourceService.getAircraftTags(), uri, MIME_TYPE_RESPONSE));
    server.registerResource("airport", new ResourceTemplate(`${RESOURCE_AIRPORTS}/{icaoCode}`, {
        list: () => ({
            resources: AeroflyFlightMcpResourceService.getAirportRessources(),
        }),
    }), {
        ...
        mimeType: MIME_TYPE_RESPONSE,
    }, (uri, { icaoCode }) => returnMcpResourceResult(AeroflyFlightMcpResourceService.getAirport(String(icaoCode)), uri, MIME_TYPE_RESPONSE));
    server.registerResource("general-rules", RESOURCE_RULES, {
...
function registerTools(server) {
    ...
    server.registerTool(TOOL_SEARCH_AIRCRAFT, {
        ...
        inputSchema: {
            query: z
                .string()
                .optional()
                .describe(`Aerofly FS 4 code, ICAO code, (partial) name of aircraft or (partial) name of livery name available for aircraft. Call ${RESOURCE_AIRCRAFT} to see a list of all available ICAO or Aerofly FS4 codes.`),
            tags: z
                .array(z.string().lowercase())
                .optional()
                .describe(`Tags like 'airliner' or 'military'. If multiple tags are submitted, the will be linked via \`OR\`. all ${RESOURCE_AIRCRAFT_TAGS} to see a list of all available tags.`),
        ...
        annotations,
    }, ({ query, tags, minimumRangeNm, minimumCruiseSpeedKts, }) => returnMcpToolSimpleResult(AeroflyFlightMcpResourceService.searchAircraft({ query, tags, minimumRangeNm, minimumCruiseSpeedKts })));
    server.registerTool(TOOL_SEARCH_AIRPORTS, {
        ...
        annotations,
    }, ({ query, geoQuery, }) => returnMcpToolSimpleResult(AeroflyFlightMcpResourceService.searchAirports({ query, geoQuery })));

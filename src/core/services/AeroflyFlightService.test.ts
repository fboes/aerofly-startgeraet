...
describe("AeroflyFlightService", () => {
    ...
    it("should calculate fuel & payload correctly and do capping", () => {
        ...
        assert.ok(
            aircraftData.maximumFuelMassKg < megaFuel,
            `Aircraft has less than ${megaFuel.toString()} of max fuel mass`,
        );
        assert.ok(aircraftData.maximumPayloadKg !== undefined);
        assert.ok(
            aircraftData.maximumPayloadKg < megaPayload,
            `Aircraft has less than ${megaPayload.toString()}kg of max payload mass`,
        );
        ...
        assert.ok(
            flight.fuelLoadSetting.fuelMass < megaFuel,
            "Even if set to beyond max fuelMass, fuelMass is capped to max fuelMass",
        );
        assert.ok(
            flight.fuelLoadSetting.payloadMass < megaPayload,
            "Even if set to beyond max payloadMass, payloadMass is capped to max payloadMass",
        );

        const maxPayload = service.getMaxRemainingPayload();
        assert.ok(
            maxPayload < aircraftData?.maximumPayloadKg,
            "Available max payload is smaller than absolute max payload due to fuel eating up the payload",
        );
        assert.ok(
            flight.fuelLoadSetting.payloadMass < aircraftData?.maximumPayloadKg,
            "Also the payload added is smaller than absolute max payload",
        );

...
const geoToUid = (lon: number, lat: number): bigint => {
  const SCALE = 1e7;
...
describe("ImportFileConverter", () => {
  ...
  it("should do some Claude tests", () => {
    ...
    for (const e of entries) {
      ...
      console.log(
        `${e.name.padEnd(16)}: ${bits.slice(0, 4)} ${bits.slice(4, 8)} ${bits.slice(8, 16)} ${bits.slice(16, 32)} ${bits.slice(32, 48)} ${bits.slice(48, 64)}`,
      );
    }
  });

import { JSONToAeroflyFlightConverter, JSONUnvalidated } from "./StringToAeroflyFlightConverter.js";
import {
    AeroflyFlight,
...
export class AeroflyCustomMissionsJsonToAeroflyFlightConverter extends JSONToAeroflyFlightConverter {
    ...
    private parseFuelLoadSettings(mission: JSONUnvalidated): AeroflySettingsFuelLoad {
    ...
    private parseNavigationConfig(mission: JSONUnvalidated): AeroflyNavigationConfig {
        return new AeroflyNavigationConfig(0, this.parseWaypoints(mission));
    }

    private parseCloudSettings(missionConditions: JSONUnvalidated): AeroflySettingsCloud[] {
    ...
    private parseAircraftSettings(mission: JSONUnvalidated): AeroflySettingsAircraft {
        const missionAircraft = this.getJSONObject(mission.aircraft);
    ...
    private parseWindSettings(missionConditions: JSONUnvalidated): AeroflySettingsWind {
    ...
    private parseTimeSettings(missionConditions: JSONUnvalidated): AeroflyTimeUtc {
        return new AeroflyTimeUtc(new Date(String(missionConditions.time ?? "")));
    }

    private parseFlightSettings(mission: JSONUnvalidated): AeroflySettingsFlight {
    ...
    private parseWaypoints(mission: JSONUnvalidated): AeroflyNavRouteBase[] {

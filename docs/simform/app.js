...
document.getElementById("wind").addEventListener("change", (event) => {
    document.getElementById("windy").value = `${String(event.currentTarget.value).padStart(
        3,
        "0",
    )}° @ ${event.currentTarget.value2}kn`;
});
...
document.getElementById("cloud").addEventListener("change", (event) => {
    ...
    document.getElementById("cloudy").value =
        `${event.currentTarget.value}% (${getCoverCode(event.currentTarget.value)}) @ ${(event.currentTarget.value2 * 100).toLocaleString()}ft`;
});
